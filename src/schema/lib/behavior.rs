// Handwritten operational behavior for the authority-verified Mentci meta-policy Interface.
//
...
wire_enum!(z2VXjF { unit {  } unary { 0 => z2VXSA(z2Vf6b ) : "Configure" } });
wire_enum!(z2VV4Q { unit {  } unary { 0 => z2VbZm(z2VUw2 ) : "OperationUnimplemented", 1 => z2VcvR(z2VbYc ) : "ConfigurationApplied", 2 => z2VNX3(z2VSVQ ) : "ConfigurationRefused" } });
wire_external_newtype!(z2VNuw, std::string::String);
wire_external_newtype!(z2VbaN, std::string::String);
wire_external_newtype!(z2Vd7N, u64);
wire_enum!(z2Vc6q { unit { 0 => z2Vbzp : "Mentci", 1 => z2VRy9 : "Criome", 2 => z2VS2U : "MetaCriome", 3 => z2VPvY : "MetaMentci", 4 => z2VMuY : "Introspect", 5 => z2VSJs : "MetaIntrospect" } unary {  } });
wire_struct!(z2Vf1e { field_0: z2Vc6q, field_1: signal_standard::schema::lib::z2VduW });
wire_struct!(z2Vezw { field_0: z2VNuw, field_1: signal_standard::schema::lib::z2VWWD, field_2: z2VbaN });
wire_enum!(z2VRQy { unit { 0 => z2VTjW : "StatusBar", 1 => z2Vdry : "Popup", 2 => z2VXwE : "Email" } unary {  } });
wire_struct!(z2Vf6b { field_0: z2Vf1e, field_1: z2Vezw, field_2: z2VRQy });
wire_struct!(z2VbYc { field_0: z2Vd7N });
wire_enum!(z2VLdm { unit { 0 => z2VNXr : "ManagerAuthorityRequired", 1 => z2VRYE : "MalformedConfiguration", 2 => z2VVgB : "StoreUnavailable" } unary {  } });
wire_struct!(z2VSVQ { field_0: z2VLdm });
wire_enum!(z2VZgH { unit { 0 => z2VT2e : "Configure" } unary {  } });
wire_enum!(z2VcWY { unit { 0 => z2VLm7 : "DependencyNotReady", 1 => z2VV66 : "NotBuiltYet" } unary {  } });
wire_struct!(z2VUw2 { field_0: z2VZgH, field_1: z2VcWY });
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)] #[repr(u8)] pub enum InputRoute { Configure }
#[derive(Clone, Copy, Debug, PartialEq, Eq)] #[repr(u8)] pub enum OutputRoute { OperationUnimplemented, ConfigurationApplied, ConfigurationRefused }

impl z2VXjF {
    pub fn route(&self) -> InputRoute { match self { Self::z2VXSA(_) => InputRoute::Configure } }
...
impl z2VV4Q {
    pub fn route(&self) -> OutputRoute { match self { Self::z2VbZm(_) => OutputRoute::OperationUnimplemented, Self::z2VcvR(_) => OutputRoute::ConfigurationApplied, Self::z2VNX3(_) => OutputRoute::ConfigurationRefused } }
...
impl signal_frame::RequestPayload for z2VXjF {}
impl signal_frame::SignalOperationHeads for z2VXjF { const HEADS: &'static [&'static str] = &["Configure"]; }
impl signal_frame::LogVariant for z2VXjF { fn log_variant(&self) -> u64 { let route = self.wire_route(); u64::from(route.root().value()) | (u64::from(route.variant().value()) << 8) } }

pub type Frame = signal_frame::BoundExchangeFrame<ContractMarker, z2VXjF, z2VV4Q>;
pub type FrameBody = signal_frame::ExchangeFrameBody<z2VXjF, z2VV4Q>;
pub type Request = signal_frame::Request<z2VXjF>;
pub type ReplyEnvelope = signal_frame::Reply<z2VV4Q>;
pub type RequestBuilder = signal_frame::RequestBuilder<z2VXjF>;

impl ContractMarker {
    pub fn decode_frame(bytes: &[u8]) -> Result<Frame, SignalFrameError> { Frame::decode(bytes).map_err(|_| SignalFrameError::ArchiveDecode) }
    pub fn decode_single_request(bytes: &[u8]) -> Result<(signal_frame::ExchangeIdentifier, z2VXjF), SignalFrameError> { match Self::decode_frame(bytes)?.into_body() { FrameBody::Request { exchange, request } => { let found = request.payloads().len(); if found != 1 { return Err(SignalFrameError::OperationCount { found }); } Ok((exchange, request.payloads.into_head())) }, _ => Err(SignalFrameError::UnexpectedFrameBody) } }
}
