...
const MODULE_PATH: &[&str] = &["meta_signal_mentci", "lib"];
...
impl SchemaBuild {
    ...
    fn run(&self) {
        ...
        let source =
            fs::read_to_string(&source_path).expect("read Mentci meta-policy Interface source");
        ...
        .expect("assemble authority-approved Mentci meta-policy Interface transaction");
        let rust = rust_logos();
        let type_paths = ProducerRustTypePaths::new();

        BootstrapInterfaceGeneration::new(&assembly, &rust, &type_paths, &source_path, &rust_path)
            .generate()
            .expect("project Mentci meta-policy Interface from the verified transaction")
            .write_or_check("META_SIGNAL_MENTCI_UPDATE_INTERFACE_ARTIFACTS")
            .expect("checked-in Mentci meta-policy Interface source and Rust projection are fresh");
        CargoEthosSourceMetadata::new("meta-signal-mentci")
...
fn imported_seats() -> Vec<ImportedSeat> {
    ...
    let wanted_0 = ["StandardSocket", "ComponentKind"]
        .into_iter()
        .collect::<BTreeSet<_>>();
    seats.extend(
...
fn bootstrap_catalog() -> BootstrapCatalog {
    ...
    .expect("owner Mentci meta-policy bootstrap catalog is complete")
}
